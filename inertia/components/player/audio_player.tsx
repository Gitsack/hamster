...
export function AudioPlayer() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-20 bg-sidebar border-t border-sidebar-border z-50">
      <div className="h-full px-4 flex items-center gap-4">
        {/* Track info */}
        <div className="flex items-center gap-3 w-40 md:w-64 min-w-0">
        ...
        <div className="flex-1 flex flex-col items-center gap-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="icon-sm" aria-label="Previous track" onClick={previous}>
            ...
            <Button variant="ghost" size="icon-sm" aria-label="Next track" onClick={next}>
            ...
            <Button
              variant="ghost"
              size="icon-sm"
              aria-label={repeatMode === 'one' ? 'Repeat track' : 'Repeat'}
              aria-pressed={repeatMode !== 'none'}
              className={cn('hidden sm:inline-flex', repeatMode !== 'none' && 'text-primary')}
          ...
          <div className="w-full max-w-md flex items-center gap-2">
            <span className="readout text-xs text-muted-foreground w-10 text-right tabular-nums">
            ...
            <Slider
              ...
              className="flex-1"
            />
            <span className="readout text-xs text-muted-foreground w-10 tabular-nums">
              {formatTime(duration)}
            </span>
          </div>
        </div>
