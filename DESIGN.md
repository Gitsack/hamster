...
The frame is a persistent left rail plus a single scrolling work area. The sidebar is 16rem expanded, 3rem collapsed to icons, and 18rem when it becomes an overlay sheet below 768px; its state persists in a cookie and toggles with ⌘/Ctrl+B. The header is a sticky 56px minimum bar carrying the sidebar trigger, a vertical hairline, the page title, and a right-aligned action cluster that wraps rather than truncating. It rides inside the scrolling work area: below 768px it slides out of the way once the page has scrolled 96px down and returns the moment the operator scrolls up, so breadcrumbs, filters and page actions are always one short swipe away rather than one long scroll back to the top. From 768px up there is room to spare, so it simply stays pinned. Content is padded 16px and, when the audio player is mounted, gains 96px of bottom clearance so the last row is never trapped behind it.
...
- **On touch, the first tap arms it.** A phone has no hover, and the swap carries the only warning there is, so a coarse pointer gets the same face by a different trigger: the first tap arms the badge — identical Alarm Red fill, identical consequence icon and label — and a second tap within three seconds commits. Moving focus away, or three seconds of silence, stands it back down. Same pixels, same colour, same words; only the trigger differs. A hover-only destructive control is an unfinished control, not a desktop one.
- **No handler, no threat.** A badge given no reversal action renders as a plain statement of state — no swap, no Alarm Red, no destructive tooltip. A control that shows a consequence and then does nothing is worse than a label.
